...
public struct ShapeAdjacentCoords {
    ...
    init(shape: Polygon) {
        adjacents = (0..<shape.edges.count).map { i in
            ...
            return Self.getAdjacentCoords(shape: shape, edge: edge, normal: normal)
        }
    }
    
    internal static func getAdjacentCoords(shape: Polygon, edge: Polygon.Segment, normal: Vector) -> [Coord] {
        ...
        if abs(dx) < abs(dy) {
            return getAdjacentCoordsAlongY(shape: shape, edge: edge, normal: normal)
        }
        return getAdjacentCoordsAlongX(shape: shape, edge: edge, normal: normal)
    }
    
    internal static func getAdjacentCoordsAlongY(shape: Polygon, edge: Polygon.Segment, normal: Vector) -> [Coord] {
        ...
        let points: [ShapeCoord] = stride(from: y0, through: yf, by: step).compactMap { y in
            let x = x0 + dx * (y - y0) / dy
            let ix = Int(round(x))
            let iy = Int(round(y))
    ...
    internal static func getAdjacentCoordsAlongX(shape: Polygon, edge: Polygon.Segment, normal: Vector) -> [Coord] {
        let n = normal.unit().scaled(0.5)
        let p0 = Vector(edge.p0).adding(n)
        let pf = Vector(edge.pf).adding(n)
        ...
        let points: [ShapeCoord] = stride(from: x0, through: xf, by: step).compactMap { x in
            let y = y0 + dy * (x - x0) / dx
            let ix = Int(round(x))
            let iy = Int(round(y))
            // What to do when the computed point lies inside shape?
