...
final class BGKCollisionTests: XCTestCase {
    ...
    func testDirection(direction: Int, windSpeed: Double) throws {
        let width = 5
        let height = 2
        var n = LatticeNodeData.allocate(capacity: numDirections * width * height)
        // Initialize nodes to equilibrium condition for given wind-from-west.
        Lattice.initNodeArray(n: &n, width: width, height: height, windSpeed: windSpeed)
        ...
        let equil = BGKCollision.getFEq(
            direction: direction, siteProps: props, dvix: dvx[direction], dviy: dvy[direction])
